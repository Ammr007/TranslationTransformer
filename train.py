...
def run_validation(model, validation_ds, tokenizer_src, tokenizer_tgt, max_len, device,
    ...
    with torch.no_grad():
        for batch in validation_ds:
            ...
            writer.add_text(f'validation/source_{count}', source_text, global_step)
            writer.add_text(f'validation/target_{count}', target_text, global_step)
            writer.add_text(f'validation/predicted_{count}', model_out_text, global_step)
...
def get_ds(config):
    ds_raw = load_dataset("ai4bharat/samanantar", config["lang_tgt"], split="train")
    ...
    train_ds = LanguageDataset(train_ds_raw, tokenizer_src, tokenizer_tgt, config['seq_len'])
    val_ds = LanguageDataset(val_ds_raw,   tokenizer_src, tokenizer_tgt, config['seq_len'])
